...
function langInstruction(lang?: string): string {
  ...
  return ` Respond entirely in ${names[lang] ?? lang}.`;
}

router.post("/botanical-insight", async (req, res) => {
  const { plantName, nativeRegion, lang } = req.body as { plantName?: string; nativeRegion?: string; lang?: string };
  if (!plantName) {
...
1. INSIGHT: A vivid, surprising botanical fact in 2-3 sentences. Reveal something most people have never heard — an evolutionary trick, a chemical secret, an unexpected historical use, or a bizarre survival strategy. Write as if telling a friend the most amazing thing you just learned. Be specific, sensory, and memorable. No markdown, no bullet points.
...
SCIENTIFIC: [scientific name here]`;
    ...
    const openaiRes = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: `You are a passionate botanist who makes plant facts feel magical and surprising. Follow the format precisely.${langInstruction(lang)}` },
...
router.post("/botanical-quiz", async (req, res) => {
  const { plantName, scientificName, lang } = req.body as {
    plantName?: string;
...
Rules: question is interesting botanical trivia, options has exactly 4 choices, correct is 0-3 index of right answer, explanation is 1-2 sentences.`;
    ...
    const openaiRes = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: `You are a botanist quiz master. Return only JSON.${langInstruction(lang)}` },
          { role: "user", content: prompt },
        ],
        max_tokens: 300,
      }),
...
router.post("/botanical-story", async (req, res) => {
  ...
  const scientificPart = scientificName ? ` (${scientificName})` : "";
  const prompt = `Write a captivating 4-part botanical story about "${plantName}"${scientificPart}.
Be vivid, specific, and surprising — each section should feel like a mini-revelation.
Return ONLY valid JSON with no markdown, no explanation:
{
  "etymology": "2-3 sentences. Reveal the surprising Latin/Greek roots of both the common name and scientific name. Include who named it and the story behind that choice.",
  "history": "2-3 sentences. Highlight the most surprising historical or medicinal use — something that would genuinely astonish someone. Be specific with time periods and civilizations.",
  "folklore": "2-3 sentences. Share the most evocative myth, legend, or belief. Use vivid sensory language that makes the folklore feel real and magical.",
  "ecology": "2-3 sentences. Describe the most remarkable adaptation, pollinator relationship, or survival trick — something that reveals hidden engineering in nature."
}`;
  ...
  try {
    const openaiRes = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: `You are a botanical storyteller. Return only valid JSON with no markdown.${langInstruction(lang)}` },
          { role: "user", content: prompt },
        ],
        max_tokens: 520,
      }),
  ...
  try {
    const openaiRes = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: `You are an expert botanist and plant care specialist. Return only JSON.${langInstruction(lang)}` },
          { role: "user", content: prompt },
        ],
        max_tokens: 350,
        temperature: 0.3,
